	...
	for (cur_node = racine; cur_node; cur_node = cur_node->next) {
		...
		if (nom_noeud != NULL) {
			if (cur_node->type == XML_ELEMENT_NODE
					&& !strcmp((char*) nom_noeud, nom_table)) {
...
int inscrire_table(char* nom_table, char* nombre_attribut) {
	...
	if (doc == NULL) {
		...
		xmlFreeDoc(doc);
		printf(
				"La table n'a pas pu etre inscrite dans le fichier de meta données\n");
		return EXIT_FAILURE;
...
void appel_sup_enreg(xmlDocPtr doc, xmlNodePtr racine, char*nom_table,
		char*nom_champ, char*valeur, char*operateur, int compteur) {/***realise un appel de la fonction de suppression chaque fois qu'un nouveau record repond au critere de comparaison*/
	...
	for (i = 0; i < compteur; i++) {
		...
		supprimer_enregistrement(doc, racine, nom_table, nom_champ, valeur,
				operateur, &nombre_tuples);
	}
	tuples = maj_nombre_tuple(nom_table, racine_meta_donnees, &tuples);

	if (tuples != NULL) {

		sprintf(valeur_char, "%d",
				atoi((char*) xmlNodeGetContent(tuples)) - nombre_tuples); //conversion du nombre de tuples en char*
...
void retourne_fils_struct(xmlNodePtr noeud_struct, attribut**liste) {
	xmlNodePtr cur_node = NULL;
	for (cur_node = noeud_struct->children; cur_node; cur_node =
			cur_node->next) {
		if (cur_node->type == XML_ELEMENT_NODE) {
			attribut* nouvel_attribut = (attribut*) malloc(sizeof(attribut));
...
xmlNodePtr retourne_noeud_a_supprimer(xmlNodePtr racine, char*valeur,
		char*nom_champ, xmlNodePtr* noeud, char*operateur) {
	xmlNodePtr cur_node = NULL;
	for (cur_node = racine->children; cur_node; cur_node = cur_node->next) {
		...
		if (cur_node->type == XML_ELEMENT_NODE) {
			if (!strcmp(operateur, "=")) { //si l'operateur de comparaison est l'egalité
				if (!strcmp((char*) xmlNodeGetContent(cur_node), valeur)
						&& !strcmp((char*) cur_node->name, nom_champ)) {/**si le champ courant 													une valeur egale à  celle entrée parl'utilisateur*/
					*noeud = cur_node;
					break;
				}
			} else { //si l'operateur de comparaison est la difference
				if (strcmp((char*) xmlNodeGetContent(cur_node), valeur)
						&& !strcmp((char*) cur_node->name, nom_champ)) {/**si le champ courant a  une 														valeur   differente de  celle  entrée par l'utilisateur*/
					*noeud = cur_node;
					break;
				}
			}
		...
		retourne_noeud_a_supprimer(cur_node->children, valeur, nom_champ, noeud,
				operateur); //appel recursif de la fonction
	}
	return *noeud;
}
int supprimer_enregistrement(xmlDocPtr doc, xmlNodePtr racine, char*nom_table,
		char*nom_champ, char*valeur, char*operateur, int*nombre_tuples) {/**fonction de suppression d'un record***/
	...
	for (cur_node = racine; cur_node; cur_node = cur_node->next) {
		if (cur_node->type == XML_ELEMENT_NODE) {
			if (!strcmp((char*) cur_node->name, "record")) {/**si le noeud courant est un enregistrement**/
				...
				noeud_a_supprimer = retourne_noeud_a_supprimer(cur_node, valeur,
						nom_champ, &noeud_a_supprimer, operateur);/**recuperation du noeud à supprimer**/
				if (!strcmp(operateur, egalite)
						&& xmlNodeGetContent(noeud_a_supprimer) != NULL) {/**si l'operateur de  comparaison est l'egalité*/

					if (!strcmp((char*) xmlNodeGetContent(noeud_a_supprimer),
							valeur)) {/**si la condition d'egalité est verifiée*/
				...
				if (!strcmp(operateur, difference)
						&& xmlNodeGetContent(noeud_a_supprimer) != NULL) { /**si l'operateur de comparaison est la difference*/
					if (strcmp((char*) xmlNodeGetContent(noeud_a_supprimer),
							valeur)) {/**si la condition de difference est verifiée*/
		...
		supprimer_enregistrement(doc, cur_node->children, nom_table, nom_champ,
				valeur, operateur, nombre_tuples);
	}
}
xmlNodePtr maj_nombre_attribut(char*nom_table, xmlNodePtr racine,
		xmlNodePtr* tuples) { /**revoie l'adresse en memoire du noeud qui sera mis à jour*/
	xmlNodePtr cur_node = NULL;
	char* valeur_char;
	int valeur_int;
	for (cur_node = racine; cur_node; cur_node = cur_node->next) {
...
int supprimer_champ(xmlDocPtr doc, xmlNodePtr racine, char*nom_table,
		char*attrib) //permet de supprimer un champ
		{
...
int ajouter_champ(xmlDocPtr doc, xmlNodePtr racine, char*nom_table,
		char*attribut_type) {
...
int commande_alter(char*nom_table, char*nom_champ, char*nom_operation) //commande permettant de modifier la structure d'une table
	...
	if (booleen != 1)
		message_d_erreur("La table n'existe pas");
	else {
		...
		while (liste->attribut_suivant != NULL) {
			if (strcmp(liste->nom_attribut, nom_champ)) {
				if (!strcasecmp(nom_operation, "ADD")) { //si l'action concernée par la modification est un  ajout
					...
					if (tuples != NULL) {
						sprintf(valeur_char, "%d",
								atoi((char*) xmlNodeGetContent(tuples)) + 1); //conversion du nombre de tuples en char*
						xmlNodeSetContent(tuples,BAD_CAST valeur_char); //application la  nouvelle valeur du noeud
						xmlSaveFile("metaDonnees.xml", doc); //sauvegarde des modifications apportées au fichier de meta  données
						printf(
...
int commande_delete(char*nom_table, char*operateur, char*nom_champ, char*valeur) //commande permettant de supprimer un enregistrement
	...
	if (booleen != 1)
		...
		booleen = chercher_nombre_tuples(nom_table, retourne_racine(),
				&booleen);
		while (liste->attribut_suivant != NULL) {
			if (!strcmp(liste->nom_attribut, nom_champ)) { /**si l'attrbut existe*/
				appel_sup_enreg(doc, racine, nom_table, nom_champ, valeur,
						operateur, booleen);
				break;
			}
			liste = liste->attribut_suivant;
...
int selectionne_noeud(xmlNodePtr racine, char* nom_table, int*compteur) //affiche le resultat d'une selection
	...
	for (cur_node = racine; cur_node; cur_node = cur_node->next) {
		if (cur_node->type == XML_ELEMENT_NODE
				&& !strcmp((char*) cur_node->name, "record")) {
			// if() {
			if (*(compteur) == 0) {
				int nombre_champ = 0; //contiendra le nombre d'attribut de la table
				int i = 0;
				chercher_nombre_attribut(nom_table, retourne_racine(),
						&nombre_champ); //recupere le nombre d'attribut de la table
				struct attribut** tableau = (attribut**) malloc(
						nombre_champ * sizeof(struct attribut*)); //tableau permettant d'inverser l'ordre des elements
				strcpy(nom_tempon, nom_table);
				xmlDocPtr doc = xmlParseFile(strcat(nom_tempon, ".xml"));
				racine = xmlDocGetRootElement(doc);
				...
				for (i = nombre_champ - 1; i >= 0; i--) {
					printf("%s(%s)         ", tableau[i]->nom_attribut,
							tableau[i]->type_attribut); //affichage de l'entete ie les champs et leur type
				}
			}
			printf("\n");
 ...
 la selection d'un  seul  champ*/
{
	...
	if (booleen != 1)
		message_d_erreur("La  table entrée  n'existe pas");/**on teste d'abord si la table existe*/
	else {
		attribut* liste;
		liste->attribut_suivant = NULL;
		xmlDocPtr doc = xmlParseFile(strcat(nom_tempon, ".xml"));
		...
		while (liste->attribut_suivant != NULL) {
			...
			printf("\n");
			nombre_ligne_trouve = selectionne_noeud(racine, nom_table,
					&nombre_ligne_trouve);/**affichage du resultat de la selection*/
			if (nombre_ligne_trouve != 0)
				printf(" \n%d ligne(s) trouvée(s) \n", nombre_ligne_trouve);
			else
				printf("Aucune donnée n'a été trouvée\n");
			break;
...
xmlNodePtr maj_nombre_tuple(char*nom_table, xmlNodePtr racine,
		xmlNodePtr* tuples) {
	xmlNodePtr cur_node = NULL;
	char* valeur_char;
	int valeur_int;
...
int commande_insert(char * nom_table, char* values) {
	...
	nombre_champs = chercher_nombre_attribut(nom_table, retourne_racine(),
			&nombre_champs);        //retourne le nombre d'attributs de la table
	...
	if (booleen != 1)
		...
		if (nombre_champs != compter_champs(values)) {
			...
			message_d_erreur(
					"Le nombre de valeurs est different du nombre de champs");
			return 1;
		}
...
attribut* recuperer_structure_table(char*nom_table, xmlNodePtr racine,
		attribut**liste) {
	...
	for (cur_node = racine; cur_node; cur_node = cur_node->next) {
		...
		if (cur_node->type == XML_ELEMENT_NODE
				&& !strcmp((char*) cur_node->name, "struct")) {/**on  recupere tous les noueds entre <struct></struct>*/
...
int commande_desc(char* nom_table) {
	...
	if (verif != 1)
		message_d_erreur("La table entrée n'existe pas");
	else {
		...
		struct attribut** tableau_attribut = (attribut**) malloc(
				nb_attributs * sizeof(struct attribut*));
		liste->attribut_suivant = NULL;
		liste = recuperer_structure_table(nom_table, racine, &liste); //recuperation de la structure de la table
		while (liste->attribut_suivant != NULL) {
		...
		for (i = nb_attributs - 1; i >= 0; i--) {
			printf("               %s                      %s             \n",
					tableau_attribut[i]->nom_attribut,
					tableau_attribut[i]->type_attribut);
			printf(
					"| ____________________________________________________________ |\n");
		}
		chercher_nombre_tuples(nom_table, retourne_racine(), &nb_tuples);
		printf(
				"\n\nCette table possede %d champs et contient  %d enregistrement(s)",
				nb_attributs, nb_tuples);
	}
...
int inserer_tuple(char*nom_table, char*values, int nombre_champs) {
	...
	struct attribut** tableau_attribut = (attribut**) malloc(
			nombre_champs * sizeof(struct attribut*)); //recuperation des element pour l'inversion
	i = 0;
	...
	xmlNodePtr
	record=xmlNewNode(NULL,BAD_CAST "record"); //insertion d'un nouvel enregistrement dans la table
	...
	for (i = nombre_champs - 1; i >= 0; i--) {
		...
		if (!strcmp((char*) tableau_attribut[i]->type_attribut, "integer")) { //si le type de l'attribut courant est un entier
			if (strtol(tableau_valeur[compteur], NULL, 10) == 0
					&& strlen(tableau_valeur[compteur]) != 1) {
				printf("L'attribut %s est un entier",
						tableau_attribut[i]->nom_attribut);
				verif_type = 1;
				break;
			} else {
			...
			if (!strcmp((char*) tableau_attribut[i]->type_attribut, "text")) {/**si le type est text,aucun controle n'est necessaire*/
				...
				if (!strcmp((char*) tableau_attribut[i]->type_attribut,
						"date")) {/**on appele là lma fonction de verification de date*/
...
int verifier_date(char* chaine)/**verifie la validité d'une date **/
{
	...
	if (longueur_chaine != 8 && longueur_chaine != 10) {
		...
		if (!strtol(jour, NULL, 10) || !strtol(mois, NULL, 10)
				|| !strtol(annee, NULL, 10)) {
			verif = 1;
		} else {
			if (strtol(jour, NULL, 10) > 31 || strtol(mois, NULL, 10) > 12)
				verif = 1;
		}
...
int commande_update_sans_where(char*nom_table, char*nom_champ, char* valeur) { //lorsque la clause where est abscente,tous les records sont mis à jour 
	char nom_tempon[50];
	...
	if (!verif)
		message_d_erreur("La table n'existe pas"); //si la table est inexistante
	else {
		if (!verifier_attribut(nom_table, nom_champ))
			message_d_erreur("Le champ entré n'existe pas");
		else {
			...
			if (!strcmp(type_attribut, "integer")) {
				if (!strtol(valeur, NULL, 10) && strlen(valeur) != 1) {
					...
					printf(
							"Le champ %s est de type integer,la valeur de remplacement n'est pas entier",
							nom_champ);
					return EXIT_FAILURE;
				} else {
					...
					cherche_record(doc, nom_table, nom_champ, racine, valeur,
							&nb_tuples);
				}
			} else {
				if (!strcmp(type_attribut, "date")) {/***si l'attribut est de type date***/
					if (verifier_date(valeur)) {
						printf(
								"Le champ   %s   est de type date,la date entrée n'est pas valide",
								nom_champ);
						return EXIT_FAILURE;
					} else {
						...
						cherche_record(doc, nom_table, nom_champ, racine,
								valeur, &nb_tuples);
					}
...
void cherche_record(xmlDocPtr doc, char*nom_table, char*nom_champ,
		xmlNodePtr racine, char*valeur, int* nb_tuples) { // parcours tous les enregistrement d'une table et les passe a la fonction de mise à jour 
	xmlNodePtr cur_node = NULL;
	for (cur_node = racine; cur_node; cur_node = cur_node->next) {
		if (cur_node->type == XML_ELEMENT_NODE) {
			if (!strcmp((char*) cur_node->name, "record")) {/**si le noeud courant est un enregistrement**/
				/**appel de la fonction de remplacement*/
				maj_valeur_attribut(doc, cur_node, nom_table, nom_champ, valeur,
						nb_tuples);        //appel de la fonction de mise à jour
			}
		}
		cherche_record(doc, nom_table, nom_champ, cur_node->children, valeur,
				nb_tuples);                                     //appel recursif
	}
}
/***met a jour un ligne d'un enregistrement***/
void maj_valeur_attribut(xmlDocPtr doc, xmlNodePtr record, char* nom_table,
		char*nom_champ, char* valeur, int* nb_tuples) {
	xmlNodePtr cur_node = NULL;
	char nom_tampon[50];
	...
	for (cur_node = record->children; cur_node; cur_node = cur_node->next) {
		...
		maj_valeur_attribut(doc, cur_node->children, nom_table, nom_champ,
				valeur, nb_tuples);
	}
}
/***commande update avec la clause where**/
int commande_update_avec_where(char*nom_table, char*nom_champ1, char*nom_champ2,
		char* valeur1, char*valeur2) {
	char nom_tempon[50];
	...
	if (!verif)
		...
		if (!verifier_attribut(nom_table, nom_champ1)
				|| !verifier_attribut(nom_table, nom_champ2))
			message_d_erreur("Le champ entré n'existe pas");
		else {
			xmlDocPtr doc = xmlParseFile(strcat(nom_tempon, ".xml"));
			xmlNodePtr racine = xmlDocGetRootElement(doc);
			...
			if (!strcmp(type_attribut2, "integer") && !strtol(valeur1, NULL, 10)
					&& strlen(valeur2) != 1) {
				/**si le champ de la clause  where
				 est un  entier et la valeur de comparaison est d'un autre type*/
				printf("Le champ %s de la clause WHERE est de type integer",
						nom_champ2);
				return EXIT_FAILURE;

			}
			if (!strcmp(type_attribut2, "date") && verifier_date(valeur2)) {
				printf("Le champ %s de la clause WHERE est de type date",
						nom_champ2);
				return EXIT_FAILURE;
			}
			if (!strcmp(type_attribut1, "integer")) {
				if (!strtol(valeur1, NULL, 10) && strlen(valeur1) != 1) {
					...
					cherche_record2(doc, nom_table, nom_champ1, nom_champ2,
							racine, valeur1, valeur2, &nb_tuples);
				}
			} else {
...
void cherche_record2(xmlDocPtr doc, char*nom_table, char*nom_champ1,
		char*nom_champ2, xmlNodePtr racine, char*valeur1, char*valeur2,
		int* nb_tuples) {
	xmlNodePtr cur_node = NULL;
	int testeur = 0;
	for (cur_node = racine; cur_node; cur_node = cur_node->next) {
		if (cur_node->type == XML_ELEMENT_NODE) {
			if (!strcmp((char*) cur_node->name, "record")) {/**si le noeud courant est un enregistrement**/
				/**appel de la fonction de remplacement*/
				verifie_condition_where(cur_node, nom_champ2, valeur2,
						&testeur);
				if (testeur) {
					maj_valeur_attribut(doc, cur_node, nom_table, nom_champ1,
							valeur1, nb_tuples);
					testeur = 0;
				}
			}
		...
		cherche_record2(doc, nom_table, nom_champ1, nom_champ2,
				cur_node->children, valeur1, valeur2, nb_tuples);
	}
}
/***verifie la condition de la clause where**/
void verifie_condition_where(xmlNodePtr racine, char*nom_champ, char*valeur,
		int*testeur) {
...
void commande_drop(char*nom_table) {
	...
	if (!verif)
		...
		if (!remove(strcat(nom_tempon, ".xml")))
			printf("La table '%s' a été supprimée avec succes !!", nom_table);
	}
}
							...
							if (!strcasecmp(nom_commande, "op_desc")) { //aide sur la commande desc
								...
								if (!strcasecmp(nom_commande, "op_alter")) { //aide sur la commande alter
									...
									printf("La commande %s est introuvable",
											nom_commande);/*message affichée lorsque la commande saisie est introuvable*/
								}
							}
						}
					}
...
int fonction_avg(char*nom_table, char*nom_champ) {
	...
	if (!verif)
		...
		if (!verifier_attribut(nom_table, nom_champ)) {
			printf("Le champ %s n'existe pas dans la  table %s", nom_champ,
					nom_table);
			return EXIT_FAILURE;
		} else {
			xmlDocPtr doc = xmlParseFile(strcat(nom_tempon, ".xml"));
			...
			chercher_nombre_attribut(nom_table, retourne_racine(),
					&nb_attributs);
			recuperer_structure_table(nom_table, racine, &liste);
			attribut** tableau_valeur = (attribut**) malloc(
					nb_attributs * sizeof(attribut*));
			while (liste->attribut_suivant != NULL) {
			...
			for (compteur = nb_attributs - 1; compteur >= 0; compteur--) {
				if (!strcmp(tableau_valeur[compteur]->nom_attribut, nom_champ)
						&& strcmp(tableau_valeur[compteur]->type_attribut,
								"integer")) {
					printf(
							"Les champs consernés par la fonction d'agregation sont de type integer");
					return EXIT_FAILURE;
				}
				if (!strcmp(tableau_valeur[compteur]->nom_attribut, nom_champ)
						&& !strcmp(tableau_valeur[compteur]->type_attribut,
					...
					double somme_val = 0;
					int nb_tuples = 0;
					chercher_nombre_tuples(nom_table, retourne_racine(),
							&nb_tuples);
...
int fonction_sum(char*nom_table, char*nom_champ) {
	...
	si_table_existe(nom_table, retourne_racine(), &verif);
	if (!verif)
		message_d_erreur("La table enttrée n'existe pas");
	else {
		if (!verifier_attribut(nom_table, nom_champ)) {
			printf("Le champ %s n'existe pas dans la  table %s", nom_champ,
					nom_table);
			...
			for (compteur = nb_attributs - 1; compteur >= 0; compteur--) {
				if (!strcmp(tableau_valeur[compteur]->nom_attribut, nom_champ)
						&& strcmp(tableau_valeur[compteur]->type_attribut,
								"integer")) {
					printf(
							"Les champs consernés par la fonction d'aggregation sont de type integer");
					return EXIT_FAILURE;
				}
				if (!strcmp(tableau_valeur[compteur]->nom_attribut, nom_champ)
						&& !strcmp(tableau_valeur[compteur]->type_attribut,
...
void max(xmlNodePtr racine, char* nom_champ, int*val_max) {
	...
	for (cur_node = racine; cur_node; cur_node = cur_node->next) {
		if (cur_node->type == XML_ELEMENT_NODE
				&& !strcmp((char*) cur_node->name, nom_champ)
				&& !strcmp(cur_node->parent->name, "record")) {
